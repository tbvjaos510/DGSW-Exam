...
쓰레드 내에서 return (void \*)42를 했고, pthread_join으로 &returnValue를 적었다면
...
- int ledControl(int gpio,int d_time) //d_time은 ms임

- char *transString(char *str,const char *target,const char *new)

- void sig_handler(int signo)

※ pid_t, pthread_t 같이 \_t가 붙은 자료형들은 <sys/type.h>에 선언됨

※ \_t 타입 중에서 주소가 아닌 값을 넘겨주는 함수는 wait, waitpid, pthread_join 뿐이다.
